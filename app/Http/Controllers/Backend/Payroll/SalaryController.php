        ...
        try {
            if ($request->is_export) {
                try {
                    ...
                    $commissions            = $this->salaryRepository->commissions();
                    $data['additions']      = $commissions['additions'];
                    $data['deductions']     = $commissions['deductions'];
                    $data['salaryGenerates'] = $this->salaryRepository->salaryGenerates(true);

                    $fileName = 'Salary-Report-' . time() . '.xlsx';
                    $data['view'] = 'backend.payroll.salary.exports.salary-report';
                    return Excel::download(new ExportExcelService($data), $fileName);
            ...
            $data = $this->salaryRepository->salaryReport();

            $data['title'] = _trans('payroll.Salary Report');

            return view('backend.payroll.salary.salary-report', compact('data'));
