...
class ExpireNotificationController extends Controller
{
    ...
    {
        // $users = User::select('id', 'name', 'email', 'phone')->get()->toArray();
        ...
        $users  = User::query()
                    ->with('company:id')
                    ->when($isSingleDBTenantCompany, function ($q) {
                    $q->where('company_id', auth()->user()->company_id);
                })
                ->select('id', 'name', 'email', 'phone', 'company_id')
                ->get()
                ->map(function ($item) use ($isSingleDBTenantCompany, $isMultiDBTenantCompany) {
                    if ($isSingleDBTenantCompany) {
                        $item->company_user_id = $item->company_id . '' . $item->id;
                    } else if ($isMultiDBTenantCompany) {
                        $item->company_user_id = $item->company_id . '' . $item->id;
                    } else {
