...
class SubscriptionModuleValueRepository implements SubscriptionModuleValueRepositoryInterface
{

    use AuthorInfoTrait, RelationshipTrait, RelationCheck, ApiReturnFormatTrait;
    protected $model;

    public function __construct(SubscriptionModuleValue $model)
    {
        $this->model = $model;
    }
    ...
    public function table($request)
    {
        ...
        return [
            'data' => $data->map(function ($data) {
                $action_button = '';
                if (hasPermission('subscription_module_values_update')) {
                    $action_button .= actionButton(_trans('common.Edit'), 'mainModalOpen(`' . route('saas.subscription_module_values.edit_modal', $data->id) . '`)', 'modal');
                }
                if (hasPermission('subscription_module_values_delete')) {
                    $action_button .= actionButton(_trans('common.Delete'), '__globalDelete(' . $data->id . ',`admin/saas/module-values/delete/`)', 'delete');
                }
