...
class SubscriptionPurchasedPackageRepository implements SubscriptionPurchasedPackageRepositoryInterface
{
    ...
    public function fields()
    {
        ...
        return [
            _trans('common.ID'),
            _trans('common.Name'),
            _trans('common.Package Plan'),
            _trans('common.Payment Method'),
            _trans('common.Currency'),
            _trans('common.Invoice No.'),
            _trans('common.Purchased Date'),
            _trans('common.Activated Date'),
            _trans('common.Expired Date'),
            _trans('common.Payment Details'),
            _trans('common.Payment Status'),
            _trans('common.Trial'),
    ...
    public function table($request)
    {
        ...
        return [
            'data' => $data->map(function ($data) {
                ...
                $packageImage = '';
                $packageImage .= '<img data-toggle="tooltip" data-placement="top" title="' . $data->image . '" src="' . uploaded_asset($data->image_id) . '" class="staff-profile-image-small" >';

                return [
                    'id' => $data->id,
                    // `title`, `code`, `symbol`, `exchange_rate`, `position`, `precision`, `status_id`

                    // "id", "user_id", "package_plan_id", "payment_method_id", "currency_id", "invoice_no", "purchased_date", "activated_date", "expired_date","is_trial", "payment_status","status_id", "action"

                    'user_id' => @$data->user->name,
                    'package_plan_id' => @$data->packagePlan->plan->title,
                    'payment_method_id' => @$data->paymentMethod->name,
                    'currency_id' => @$data->currency->title,
                    'invoice_no' => @$data->invoice_no,
                    'purchase_date' => @$data->purchase_date,
                    'activated_date' => @$data->activated_date,
                    'expired_date' => @$data->expired_date,
                    'subscription_payment_details' => @$data->subscription_payment_details,
                    'payment_status' => $data->payment_status == 0 ? '<small class="badge badge-danger">Failed</small>' :
                    ($data->payment_status == 1 ? '<small class="badge badge-success">Yes</small>' :
                        ($data->payment_status == 2 ? '<small class="badge badge-warning">Pending</small>' :
                            ($data->payment_status == 3 ? '<small class="badge badge-warning">Expired</small>' : ''))),
                    'is_trial' => @$data->is_trial ? '<small class="badge badge-success">Yes</small>' : '<small class="badge badge-danger">No</small>',
