...
class SubscriptionModuleCategoryRepository implements SubscriptionModuleCategoryRepositoryInterface
{
        ...
        return [
            'data' => $data->map(function ($data) {
                ...
                if (hasPermission('model_update')) {
                    $action_button .= '<a href="' . route('saas.subscription_module_categories.edit', [$data->id]) . '" class="dropdown-item">' . _trans('common.Edit') . '</a>';
                }
                if (hasPermission('model_delete')) {
                    $action_button .= actionButton(_trans('common.Delete'), '__globalDelete(' . $data->id . ',`admin/saas/module-categories/delete/`)', 'delete');
                }
