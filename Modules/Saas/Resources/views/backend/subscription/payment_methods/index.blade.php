                            ...
                            <div class="align-self-center">
                                <label>
                                    ...
                                    <select class="form-select d-inline-block" id="entries"
                                        onchange="get_subscription_payment_methods_datatable()">
                                ...
                                @if (hasPermission('designation_create'))
                                    <div class="align-self-center">
                                        <a href="javascript:;" role="button" class="btn-add" data-bs-toggle="tooltip"
                                            onclick="mainModalOpen(`{{ route('saas.subscription_payment_methods.create_modal') }}`)"
                            ...
                            <div class="align-self-center">
                                <div class="search-box d-flex">
                                    <input class="form-control" placeholder="{{ _trans('common.Search') }}" name="search"
                                        onkeyup="get_subscription_payment_methods_datatable()" autocomplete="off">
...
@section('script')
    ...
    <script>
        function get_subscription_payment_methods_datatable(...values) {
            var tableUrl = $('meta[name="base-url"]').attr("content");
            let data = [];
            let url = tableUrl + "/admin/saas/payment-methods";
            data["url"] = url;
            ...
            data["column"] = ["id", "name", "charge", "test_mode", "data", "currency_id", "status", "action"];

            data["table_id"] = "subscription_payment_methods_datatable";
            table(data);
        }
        $(".subscription_payment_methods_datatable").length > 0 ? get_subscription_payment_methods_datatable() : "";
    </script>
@endsection
